...
export async function GET(
  ...
  const requestId = resolveRequestId(_req.headers);
  const startedAt = Date.now();

  try {
    const auth = await requireAuth();
    if (!auth.success) {
      return unauthorizedInviteError("invite.list", requestId, auth.response.status);
    }
    ...
    if (!membership) {
      return apiError("Not found", 404, {
        supportCode: getSupportCode("invite.list", "NOT_FOUND"),
        requestId,
      });
    }
    ...
    if (membership.role !== "ORG_ADMIN") {
      return apiError("Insufficient permissions", 403, {
        supportCode: getSupportCode("invite.list", "FORBIDDEN"),
        requestId,
      });
    }
    ...
    if (error instanceof InvitationsApiError) {
      ...
      return mapInviteError(error, requestId, "invite.list");
    }
    ...
    return apiError("Internal server error", 500, {
      code: "SERVER_ERROR",
      supportCode: getSupportCode("invite.list", "SERVER_ERROR"),
      requestId,
    });
  }
}
...
export async function POST(
  ...
  try {
    const auth = await requireAuth();
    if (!auth.success) {
      return unauthorizedInviteError("invite.create", requestId, auth.response.status);
    }
    ...
    if (!membership) {
      return apiError("Not found", 404, {
        supportCode: getSupportCode("invite.create", "NOT_FOUND"),
        requestId,
      });
    }
    ...
    if (membership.role !== "ORG_ADMIN") {
      return apiError("Insufficient permissions", 403, {
        supportCode: getSupportCode("invite.create", "FORBIDDEN"),
        requestId,
      });
    }
    ...
    try {
      ...
      return apiError("Invalid JSON body", 400, {
        ...
        supportCode: getSupportCode("invite.create", "INVITE_CONFLICT"),
        requestId,
      });
    }
    ...
    if (idempotencyKey) {
      ...
      if (claim.state === "in-progress") {
        return apiError("Request with this idempotency key is already in progress", 409, {
          ...
          supportCode: getSupportCode("invite.create", "IDEMPOTENCY_IN_PROGRESS"),
          requestId,
    ...
    try {
      ...
      if (error instanceof InvitationsApiError) {
        const mapped = buildInviteError(error, requestId, "invite.create");
    ...
    if (error instanceof InvitationsApiError) {
      ...
      return mapInviteError(error, requestId, "invite.create");
    }
    ...
    return apiError("Internal server error", 500, {
      code: "SERVER_ERROR",
      supportCode: getSupportCode("invite.create", "SERVER_ERROR"),
      requestId,
    });
  }
}
